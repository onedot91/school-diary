...
import { Pencil, Save, Plus, Trash2, Calendar, BookOpen, Download, Sparkles, Wand2, Loader2 } from 'lucide-react';
...
export default function App() {
  ...
  const handleUpdate = (id: string, text: string) => {
    const replacedText = text.replace(/(\d)ㅔ/g, '$1p');
    setAnnouncements(announcements.map(a => a.id === id ? { ...a, text: replacedText } : a));
  };
  ...
  const handleComplete = async () => {
    ...
    const textsToCorrect = announcements.filter(a => a.text.trim() !== '');
    ...
    if (textsToCorrect.length > 0) {
      ...
      try {
        ...
        const response = await ai.models.generateContent({
          ...
          contents: `다음은 초등학교 선생님이 작성한 알림장 내용입니다. 초등학생이 읽기 편한 다정하고 친근한 말투(해요체/하십시오체 등 기존 어조 유지)를 훼손하지 않으면서, 오탈자와 띄어쓰기, 맞춤법만 정확하게 교정해주세요.\n\n${JSON.stringify(textsToCorrect.map(a => ({ id: a.id, text: a.text })))}`,
          config: {
            ...
            responseSchema: {
              ...
              items: {
                ...
                properties: {
                  id: { type: Type.STRING },
                  text: { type: Type.STRING, description: "맞춤법이 교정된 텍스트" }
                },
                required: ["id", "text"]
              }
            }
        ...
        if (response.text) {
          ...
          setAnnouncements(prev => prev.map(a => {
            const corrected = correctedData.find((c: any) => c.id === a.id);
            return corrected ? { ...a, text: corrected.text } : a;
          }));
        }
      } catch (error) {
  ...
  const handleSaveImage = async () => {
    ...
    try {
      ...
      const formattedDate = match 
        ...
        : date.replace(/[^0-9]/g, '');
      const filename = `${formattedDate || '오늘'}_알림장.png`;
  ...
  // Dynamic sizing based on item count to prevent scrolling
    ...
    <div className="h-screen p-3 md:p-6 text-[#2c1e16] flex flex-col items-center overflow-hidden">
      ...
      <div className="w-full max-w-5xl flex justify-between items-end mb-4 gap-4 shrink-0">
        <div className="flex items-center gap-4">
          {/* Bear Character */}
          <div className="relative w-16 h-16 md:w-24 md:h-24 hover:-translate-y-1 transition-transform duration-300 shrink-0">
            <img 
              src="/bear.png" 
              alt="알림장 곰돌이" 
          ...
          <div className="flex flex-col justify-center">
            <h1 className="text-4xl md:text-6xl text-[#8b5a2b] drop-shadow-sm tracking-wide font-bold" style={{ textShadow: '1px 1px 0px rgba(74, 55, 40, 0.2)' }}>오늘의 알림장</h1>
          </div>
        </div>
        ...
        <div className="flex gap-3 shrink-0">
          ...
          <button 
            ...
            className={`flex items-center gap-1.5 px-5 py-2.5 rounded-2xl text-lg md:text-xl shadow-sm transition-all hover:-translate-y-0.5 ${
              isEditing 
                ? 'bg-[#5C8D6D] text-white hover:bg-[#4a7258] border-2 border-[#4a7258] disabled:opacity-80 disabled:cursor-wait disabled:hover:translate-y-0' 
                : 'bg-[#A67C52] text-white hover:bg-[#8b6844] border-2 border-[#8b6844]'
            }`}
          >
            {isEditing ? (
              isCorrecting ? <><Loader2 className="w-5 h-5 animate-spin" /> 정리 중...</> : <><Save className="w-5 h-5" /> 다 썼어요!</>
            ) : (
              <><Pencil className="w-5 h-5" /> 선생님 쓰기</>
            )}
          </button>
        </div>
      </div>
        ...
        <div className="pl-16 pr-8 py-8 md:pl-24 md:pr-12 md:py-10 flex-1 flex flex-col relative z-10 h-full">
          ...
          <div className="flex flex-col gap-0 flex-1 justify-start">
            {visibleAnnouncements.map((announcement, index) => (
              <div key={announcement.id} className={`flex items-start group border-b-2 border-[#f0eadd] ${sizes.minH} ${sizes.py} shrink-0`}>
                <span className={`${sizes.text} text-[#A67C52] w-10 md:w-14 shrink-0 mt-1 font-bold`}>{index + 1}.</span>
                ...
                {isEditing ? (
                  ...
                  <p className={`flex-1 ${sizes.text} text-[#2c1e16] leading-relaxed md:leading-relaxed mt-1 break-keep tracking-wide`}>
                    {announcement.text}
                  </p>
                )}
              </div>
            ...
            {isEditing && (
              <div className={`flex items-center ${sizes.minH} mt-2 shrink-0`}>
                ...
                >
                  <Plus className="w-6 h-6 md:w-7 md:h-7" /> 새로운 내용 추가하기
                </button>
              </div>
            )}

            {/* Closing Message Rendered as a List Item */}
            {!isEditing && showClosing && (
              <div className={`flex items-start group border-b-2 border-[#f0eadd] ${sizes.minH} ${sizes.py} shrink-0`}>
                <span className={`${sizes.text} text-[#A67C52] w-10 md:w-14 shrink-0 mt-1 font-bold`}>{visibleAnnouncements.length + 1}.</span>
            ...
            {/* Button to reveal closing message */}
            {!isEditing && !showClosing && (
              <div className="mt-4 flex justify-center shrink-0">
                ...
                >
                  <Sparkles className="w-5 h-5" /> 마무리 인사 보기
                </button>
              </div>
            )}
          </div>
      ...
      {/* Footer */}
      <div className="mt-4 text-[#5C8D6D] text-xl md:text-2xl flex items-center gap-2.5 bg-[#fcf8f2] px-6 py-2.5 rounded-full border border-[#e8e0d5] font-medium shadow-sm shrink-0">
